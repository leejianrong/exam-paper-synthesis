"""fractions_medium (KAN-232): fraction of a remainder (two-step). Mirrors the
ratio/percentage medium suites — schema-valid, tagged medium/complex, 3 marks,
no diagram (the bar-model check is ratio-specific), deterministic, seeds vary,
and hand-verified golden fixtures."""
...
GOLDEN = Path(__file__).parent / "golden" / "fractions_medium.jsonl"
...
def test_generate_is_schema_valid_and_tagged(seed: int):
    ...
    assert obj["schema_version"] == "1.5.0"
    assert obj["id"] == f"fractions_medium:{seed}"
    assert obj["source_type"] == "generated"
    assert obj["blueprint_code"] == "fractions_medium"
...
def test_generate_is_deterministic(seed: int):
    assert generate("fractions_medium", seed) == generate("fractions_medium", seed)


def test_different_seeds_vary():
    ids = {generate("fractions_medium", s)["id"] for s in SEEDS}
    params = [
        (
            generate("fractions_medium", s)["parameters"]["whole"],
            generate("fractions_medium", s)["parameters"]["n1"],
            generate("fractions_medium", s)["parameters"]["d2"],
        )
        for s in SEEDS
    ]
...
def test_golden_fixtures():
    ...
    solver = get_solver("fractions_medium")
