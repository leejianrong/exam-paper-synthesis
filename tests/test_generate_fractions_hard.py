"""fractions_hard (KAN-232): successive fractions of a remainder with an unknown
whole (inverse / work-backwards). Mirrors the ratio/percentage hard suites —
schema-valid, tagged hard/non-routine, 4 marks, no diagram, deterministic, seeds
...
GOLDEN = Path(__file__).parent / "golden" / "fractions_hard.jsonl"
...
def test_generate_is_schema_valid_and_tagged(seed: int):
    ...
    # Inverse question: the recovered whole exceeds what was left over.
    assert answer["value"] > obj["parameters"]["left"]
...
def test_generate_is_deterministic(seed: int):
    assert generate("fractions_hard", seed) == generate("fractions_hard", seed)


def test_different_seeds_vary():
    ids = {generate("fractions_hard", s)["id"] for s in SEEDS}
    params = [
        (
            generate("fractions_hard", s)["parameters"]["left"],
            generate("fractions_hard", s)["parameters"]["n1"],
            generate("fractions_hard", s)["parameters"]["d2"],
        )
        for s in SEEDS
    ]
...
def test_golden_fixtures():
    ...
    solver = get_solver("fractions_hard")
